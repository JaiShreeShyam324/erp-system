...
export default function StockPage() {
  ...
  const fetchStock = async () => {
    const token = localStorage.getItem('token');
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/stock`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setList(res.data);
    } catch {
  ...
  const saveStock = async () => {
    ...
    try {
      await axios.post(
        ...
        { productId: Number(productId), quantity: Number(quantity), type },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const deleteStock = async (id: number) => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/stock/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setMessage('Deleted');
      fetchStock();
  ...
  return (
    <main className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow p-6">
        ...
        <h1 className="text-2xl font-bold mt-2 mb-6">Stock Entry</h1>
        ...
        <select className="w-full border rounded-lg px-4 py-2 mb-4" value={type} onChange={(e) => setType(e.target.value)}>
          <option value="IN">IN</option>
          <option value="OUT">OUT</option>
        </select>

        <button onClick={saveStock} className="bg-black text-white px-4 py-2 rounded-lg">
          Save Stock
        </button>
                ...
                {list.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="border p-2">
                      <button onClick={() => deleteStock(item.id)} className="bg-red-500 text-white px-3 py-1 rounded">
                        Delete
                      </button>
                    </td>
                  </tr>
